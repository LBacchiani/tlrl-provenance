#!/usr/bin/env python
"""Launch the frozen DeepLTL PointWorld PPO confirmatory campaign.

The launcher records one campaign-level launch artifact plus one immutable
per-seed manifest before starting any training process.  It intentionally
limits concurrent jobs because each PPO run creates multiple environment
workers.
"""
...
PROTOCOL = ROOT / "evidence" / "deepltl_point" / "confirmatory" / "protocol" / "deepltl_point_paper_benchmark_protocol.json"
...
def main() -> None:
    ...
    parser.add_argument("--gpu-cycle", default="0,1")
    ...
    if protocol["benchmark_id"] != "deepltl-pointworld-official-eval-50":
    ...
    # Launch-time namespace check required by the frozen protocol.
    eval_seed_bases = {
        "primary_final_policy_audit": 9_400_000,
        "secondary_checkpoint_trajectory_audit": 9_500_000,
    }
    ...
    if any(base in forbidden_seed_values for base in eval_seed_bases.values()):
        raise RuntimeError(f"evaluation seed base collision: {eval_seed_bases}")
    ...
    experiment_names = {
        seed: f"v3_confirm_ppo_5m_seed_{seed}" for seed in seeds
    }
    run_roots = {
    ...
    campaign_manifest = {
        "schema_version": "tlrl-deepltl-point-ppo-confirmatory-campaign/1",
        ...
        "protocol_path": "evidence/deepltl_point/confirmatory/protocol/deepltl_point_paper_benchmark_protocol.json",
        ...
        "launch_time_seed_namespace_check": {
            ...
            "known_development_eval_seed_bases": [730_000, 830_000, 930_000],
            "known_development_checkpoint_eval_first_seed": 930_100,
            "confirmatory_eval_seed_bases_reserved": eval_seed_bases,
            "status": "passed",
        },
    ...
    for index, seed in enumerate(seeds):
        ...
        seed_manifest = {
            "schema_version": "tlrl-deepltl-point-ppo-confirmatory-seed/1",
            "status": "frozen_before_launch",
            "scope": "confirmatory_training_only_no_result_inspection",
            "campaign_id": CAMPAIGN_ID,
            "campaign_manifest_path": f"evidence/deepltl_point/confirmatory/training/{manifest_path.name}",
    ...
    def persist(status: str) -> None:
        record = {
            "schema_version": "tlrl-deepltl-point-ppo-confirmatory-launch/1",
            "status": status,
            "campaign_id": CAMPAIGN_ID,
            "campaign_manifest_path": f"evidence/deepltl_point/confirmatory/training/{manifest_path.name}",
            "campaign_manifest_sha256": manifest_digest,
            "seed_manifests": {
                str(seed): f"evidence/deepltl_point/confirmatory/training/{path.name}"
