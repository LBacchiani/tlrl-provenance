#!/usr/bin/env python
"""Exercise DeepLTL's official LetterEnv LDBA wrapper end to end.
...
``smoke_deepltl_official_ldba_env.py`` exactly, adapted for LetterEnv's
discrete grid instead of PointLtl2's continuous zone geometry.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "evidence" / "letter_env" / "validation" / "letter_env_official_ldba_env_smoke.json",
    )
    ...
    try:
        ...
        # the ordered-eventuality task "F (a & F b)": visit 'a', then 'b'.
        # LetterEnv is a discrete torus grid, so the probe teleports the agent
        # to the cell one step away from the target letter, then takes the
        # single action that lands exactly on it -- no physics involved.
        # The final letter is visited twice: reaching the accepting sink
        # state is not itself flagged accepting (confirmed empirically) --
        # the Büchi self-loop at the sink needs one more traversal to trigger
        # ``accepting=True`` and terminate, exactly mirroring PointLtl2's own
        # probe, which revisits its last target ("yellow") twice for the same
        # reason.
        observation, info = env.reset(seed=args.seed)
        ...
        for letter in ("a", "b", "b"):
            ...
            forced_action = np.array(0, dtype=env.action_space.dtype)
            observation, reward, terminated, truncated, info = env.step(forced_action)
            ...
            if actual_labels != [letter]:
                raise AssertionError(f"forced {letter!r} contact produced labels {actual_labels}")
            if truncated:
    ...
    result = {
        "schema_version": "tlrl-letter-env-official-ldba-env-smoke/1",
        "status": "passed",
