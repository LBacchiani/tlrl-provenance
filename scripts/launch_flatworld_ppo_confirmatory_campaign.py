#!/usr/bin/env python
"""Launch the frozen FlatWorld PPO confirmatory campaign.

Mirrors scripts/launch_letter_env_ppo_confirmatory_campaign.py exactly in
...
launcher itself supervises and records completion). Same two deliberate
differences from the deepltl_point precedent, both documented in the frozen
protocol itself, not just here:

- Single GPU (GPU1) for every seed, not a two-GPU cycle.
- max_concurrent defaults to 1 (fully sequential), not 2. Running multiple
  learner seeds concurrently on one GPU previously caused a multi-hour
  throughput collapse in another benchmark -- avoided here deliberately.

Like the deepltl_point/letter_env precedent, this script does not itself
check whether the development pilot's outcomes were inspected -- that is a
judgment call by the experimenter, made before running this script, recorded in
...
PROTOCOL = ROOT / "evidence" / "flatworld" / "ppo" / "confirmatory" / "protocol" / "flatworld_ppo_paper_benchmark_protocol.json"
PROTOCOL_HASH = PROTOCOL.with_suffix(PROTOCOL.suffix + ".sha256")
EVIDENCE = ROOT / "evidence" / "flatworld" / "ppo" / "confirmatory" / "training"
CAMPAIGN_ID = "flatworld_ppo_5m_confirmatory_5seed"
...
def main() -> None:
    ...
    if protocol["benchmark_id"] != "deepltl-flatworld-official-eval-49":
    ...
    # this pillar's own pilot seed.
    forbidden_seed_values = {
        int(training["development_seed_excluded_from_primary"]),
        *map(int, seeds),
    }
    if len(forbidden_seed_values) != 1 + len(seeds):
        raise RuntimeError("seed namespace collision between PPO confirmatory seeds and pilot seed")
    ...
    experiment_names = {
        seed: f"v3_confirm_ppo_5m_flatworld_seed_{seed}" for seed in seeds
    }
    run_roots = {
    ...
    campaign_manifest = {
        "schema_version": "tlrl-flatworld-ppo-confirmatory-campaign/1",
        ...
        "protocol_path": "evidence/flatworld/ppo/confirmatory/protocol/flatworld_ppo_paper_benchmark_protocol.json",
    ...
    for index, seed in enumerate(seeds):
        ...
        seed_manifest = {
            "schema_version": "tlrl-flatworld-ppo-confirmatory-seed/1",
            "status": "frozen_before_launch",
            "scope": "confirmatory_training_only_no_result_inspection",
            "campaign_id": CAMPAIGN_ID,
            "campaign_manifest_path": f"evidence/flatworld/ppo/confirmatory/training/{manifest_path.name}",
    ...
    def persist(status: str) -> None:
        record = {
            "schema_version": "tlrl-flatworld-ppo-confirmatory-launch/1",
            "status": status,
            "campaign_id": CAMPAIGN_ID,
            "campaign_manifest_path": f"evidence/flatworld/ppo/confirmatory/training/{manifest_path.name}",
            "campaign_manifest_sha256": manifest_digest,
            "seed_manifests": {
                str(seed): f"evidence/flatworld/ppo/confirmatory/training/{path.name}"
